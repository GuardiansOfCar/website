...
export default function PointSettingCreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await fetch("/admin-point/settings", {
        method: "POST",
        data: {
      ...
      if (result?.success) {
        ...
        setError(result?.message || "생성에 실패했습니다.");
      }
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1 text-sm text-gray-500 dark:text-gray-400">
          ...
          <span className="text-cyan-400">항목 생성</span>
        </div>
      </div>
