...
export const BuyGoCar = (props: { rewards?: boolean }) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/icosales/gocar/swap/amount/${wallet.coin}`)
      .then((res) => res.json())
      .then((result) => {
  ...
  const handlePurchaseClick = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/icosales/participate`, {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/icosales/step/setting`)
      .then((res) => res.json())
      .then((result) => {
    ...
    <>
      ...
      >
        <div className={"flex items-center justify-center pt-5 pb-4"}>
          <p
            className={cn(
              "text-header-3 text-center ",
              locale === "ru" && "!text-header-4"
            )}
          >
            {t("home.presalePurchase1")}
          </p>
        </div>
...
const ICOState = ({}: {}) => {
  const { data, error } = useSWR(
    ...
    (args) =>
      fetch(`${API_BASE_URL}/icosales/cumulative/status`)
        .then((res) => res.json())
        .then(async (result) => {
          return {
...
const RewardList = ({
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/rewards/list`)
      .then((res) => res.json())
      .then((result) => {
        setList(
          (
